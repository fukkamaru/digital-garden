...
件数：15件。主クラスタの配置。Cluster 06から移管された1件を含む。
...
| [construction-workforce-aging-statistics.md](../../31_Research/construction-workforce-aging-statistics.md) | 建設業の高齢化統計とPR資料での活用 | literature | true | Cluster 06の移管候補を確定。市場・PR資料での統計活用が中心であり、保存場所は変更しない |
...
| `31_Research/PowerRenameで複数ファイル名を一括変更する方法.md` | PowerRenameで複数ファイル名を一括変更する方法 | permanent | true | Cluster 13-Eで日本語名へリネームし、一括リネームの安全な手順として再構成 |
...
| `31_Research/VS Codeのユーザーインストールとシステムインストール.md` | VS Codeのユーザーインストールとシステムインストール | permanent | true | Cluster 13-Eで日本語名へリネームし、導入形態の選択基準として再構成 |
| `31_Research/VS Codeの制限モードとワークスペースの信頼.md` | VS Codeの制限モードとワークスペースの信頼 | permanent | true | Cluster 13-Eで日本語名へリネームし、信頼境界の判断手順として再構成 |
| `31_Research/VS CodeとTyporaでMarkdownを編集する使い分け.md` | VS CodeとTyporaでMarkdownを編集する使い分け | permanent | true | Cluster 13-Eで日本語名へリネームし、編集用途の選択基準として再構成 |
...
| `32_Zk/Keyboard ManagerでCaps Lockを再割り当てる方法.md` | Keyboard ManagerでCaps Lockを再割り当てる方法 | permanent | false | Cluster 13-Eで日本語名へリネームし、個人の設定例を含む再割り当て手順として再構成 |
| `32_Zk/URLエンコードとデコードの具体例.md` | URLエンコードとデコードの具体例 | permanent | false | Cluster 13-Fで日本語名へリネームし、URLの具体例として再構成 |
...
| `32_Zk/URLで日本語がエンコードされる理由.md` | URLで日本語がエンコードされる理由 | permanent | false | Cluster 13-Fで日本語名へリネームし、URLの概念説明として再構成 |
| `32_Zk/PowerToysがWindowsに標準搭載されない理由.md` | PowerToysがWindowsに標準搭載されない理由 | permanent | false | Cluster 13-Eで日本語名へリネームし、確認済み事実と推論を分離 |
| `32_Zk/winmail.datが届く原因と対処方法.md` | winmail.datが届く原因と対処方法 | permanent | false | Cluster 13-Fで日本語名へリネームし、TNEF・再送判断の手順として再構成 |
...
件数：169件。内容監査前の残余ノートと、他クラスタの内容監査で主クラスタを変更したノートを含む。

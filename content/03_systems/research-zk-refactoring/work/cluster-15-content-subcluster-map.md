---
title: Research＋ZKリファクタリング：Cluster 15内容別子クラスタ台帳
aliases:
...
Cluster 15の有効対象278件を、既存・追加・保存フォルダ・過去の作業工程に依存せず、ノートの主題と利用場面で15の子クラスタへ割り当てた正本。退避済み11件は含めない。

## 分類原則

1. 各ノートは、もっとも中心的な内容・利用場面に基づき一つの子クラスタへだけ置く。
2. 保存フォルダ、既存／追加の出自、過去の監査工程は分類理由に使わない。
3. 複数テーマを含むノートは、今後そのノートを読む目的が最も近い子クラスタを主配置とする。
4. これは内容分類台帳であり、ファイル移動、YAML変更、統合・削除の指示ではない。

## 子クラスタ一覧

| ID | 内容 | 件数 | 境界 |
| --- | --- | ---: | --- |
| 15-01 | AI・知識管理の運用 | 21 | AIとの協働手順、Zettelkasten運用、公開・記録・リンク設計 |
| 15-02 | AIサービス・モデル利用 | 24 | AIサービス、モデル、料金・制限、ローカル利用の比較と運用 |
| 15-03 | 文書・データ・デジタル作業 | 25 | 文書作成、データ整理、ファイル・アプリ利用の実務 |
| 15-04 | PC・Web・ソフトウェア基盤 | 12 | PC、Windows、Web、ソフトウェア、ハードウェアの技術課題 |
| 15-05 | 製品・業務・産業知識 | 24 | 製品、製造・施工、マーケティング、業務運用 |
| 15-06 | 映像・画像・配信制作 | 13 | 画像・動画・印刷・配信・公開制作 |
| 15-07 | ゲーム・インタラクティブ娯楽 | 26 | ゲームの所持・購入・プレイ・配信準備 |
| 15-08 | 芸術・文化・創作 | 21 | 展覧会、作品鑑賞、創作、文化・宗教・食文化 |
| 15-09 | 旅行・地域・移動 | 21 | 旅行計画、地域、交通、宿泊、現地利用 |
| 15-10 | 個人の購入・サービス判断 | 18 | 価格、ポイント、契約、金融、個人向けサービスの選択 |
| 15-11 | 生活・住環境・個人運用 | 15 | 住環境、習慣、個人目標、所持・整理の運用 |
| 15-12 | 健康・医療・心理 | 18 | 健康、医療、心理、身体、衛生 |
| 15-13 | 言語・概念・基礎知識 | 17 | 語彙、表現、概念、一般的な基礎知識 |
| 15-14 | 自然科学・材料・食 | 10 | 自然現象、材料、食品、科学的比較 |
| 15-15 | 仕事・労働・制度 | 6 | 雇用、労働制度、教育制度、社会的条件 |
...
- [iPhoneの有線おすそ分け充電とレンタル端末で見送る判断](../../32_Zk/iphone-wired-charge-sharing-rental-decision.md)
- [SAKULaLaの生体認証サービスと店頭で見た記録](../../31_Research/sakulala-biometric-service-store-observation.md)
- [USBメモリを用途別に分離する管理方針](../../31_Research/usb-drive-purpose-separation-policy.md)
- [WindowsのDisk Event ID 7とユーザープロファイル変更後の再発](../../30_Inbox/クラスタ15に追加/windows-disk-event-id-7-profile-recurrence.md)
- [データセンター冷却特集における断熱製品の適合性](../../31_Research/data-center-cooling-feature-product-fit.md)
- [Windowsで特定アプリの音量を下げる](../../31_Research/windows-app-volume-adjustment.md)
- [Office AssistantのイルカとCaps Lockはどちらが嫌われるのか？](../../32_Zk/clippy-caps-lock-comparison-question.md)
- [Power Queryで入荷日付を日付型へ変換するコード](../../31_Research/power-query-receipt-date-transformation.md)
- [WordPressのリダイレクト設定方法と確認事項](../../31_Research/wordpress-redirect-configuration-options.md)
- [交流・直流とACアダプター表記の読み方](../../31_Research/ac-dc-basics-adapter-labels.md)
...
- [クリアファイルの使い分けを決める](../../32_Zk/how-to-use-clear-files-effectively.md)
...
- [特異的抵抗と非特異的抵抗の違い](../../32_Zk/specific-and-nonspecific-difference.md)
- [嵌合](../../32_Zk/mechanical-fit-and-mating.md)

...
このノートは、確認時に表示された利用状況画面をどう読むかを整理した記録である。この画面の情報は、通常のChatGPTチャット全体ではなく、**WorkとCodexの利用状況**を対象にしている。
...
|openai-docs 6回|Work / Codex 内のSkill利用|
...
このスクリーンショットは、「Obsidian作業で5.6 Sol＋推論：高を使い続けたことが費用対効果に見合っていたか」を検討するための、Work／Codex側の使用傾向を示すデータとして使える。ただし、通常Chatを含む全利用量や、すべてのバックグラウンド処理を完全に示す数値ではない。

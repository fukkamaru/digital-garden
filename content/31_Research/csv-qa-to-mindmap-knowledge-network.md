---
title: CSVのQ&Aデータをマインドマップと知識ネットワークへ変換する記録
aliases:
  - CSVのQ&Aデータをマインドマップと知識ネットワークへ変換する記録
type:
...
この作業の目的は、アップロードしたCSV「1日1ページ - 世界の教養_現代編.csv」を単なるデータ加工対象として扱うのではなく、**学習素材として活用すること**だった。
...
このノートは、各テーマについて作成したマインドマップの素材と、そこへ至る変換方針を残す作業記録である。各テーマの記述はCSVから取り出した学習内容の整理であり、事実関係を新たに調査・確定するためのノートではない。
...
CSVを更新・再アップロードしながら内容を確認した。確認時点の版には、主に次の列が存在していた。
...
途中でCSVが更新され、`罪と罰`というカテゴリーも追加された。
...
ただし途中から、CSVにない独自問題が混じったり、フロイトだけに話題が固定されたりした。そのため、
...
`Biography`などの英語の幹が何を表すかを確認し、次のように整理した。
...
# この作業で確立したマインドマップ作成方針

最終的に、単純なQ&Aの列挙ではなく、次の形式が意図に最も合うと分かった。
...
この方法は、単なるAnki用Q&Aを、将来的にはObsidian等で次のような知識ネットワークへ展開できる。
...
この作業を通じて、`category`ごとのQ&Aを単純転記せず、**意味単位へ再構成したMermaidマインドマップへ変換する**方針が固まった。また、`remarks`も補助情報として利用し、別カテゴリーと関連が強い内容は、将来の相互リンクを想定したノードとして残す。

特に「共産主義 ↔ 資本主義」のような関係では、`Subnote`ではなく**`Rivalry`など関係性を表す幹**を使う方向となった。

続ける場合は、残りのCSVカテゴリーも同じ基準で整理することで、「世界の教養_現代編」全体をAnki用Q&AからObsidian向け知識ネットワークへ変換できる。

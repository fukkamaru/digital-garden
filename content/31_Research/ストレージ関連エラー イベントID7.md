...
このノートは、Windowsの`Disk`ソースで記録されたEvent ID 7を、CF-LVのユーザープロファイル障害を読むための観測として位置付ける。発生日時・件数・関連ログ・実際の判断は、[Windowsユーザープロファイル障害・SSD不良ブロック調査まとめ](Windowsユーザープロファイル障害・SSD不良ブロック調査まとめ.md)を正本とする。

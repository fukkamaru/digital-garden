---
title: ローカルLLMの概要と、長文要約・文字起こし済みテキスト処理に必要なPC性能
aliases:
  - ローカルLLMの概要と、長文要約・文字起こし済みテキスト処理に必要なPC性能
...
このノートでは、**ローカルLLMとは何か**という基本から始まり、PCスペックごとの処理能力、長文要約、1時間程度の動画文字起こし後の要約処理まで、用途別に必要性能を整理する。PC購入の比較軸は[[ローカルLLMについて調べる]]、Windows上で文字起こしする手段は[[無題のファイル 6 1]]を参照する。

出発点は、「動画の会話を文字起こしし、内容を要約してObsidian向けMarkdownへ再構成したい」という用途だった。当初は、動画からの音声抽出、Whisperなどによる文字起こし、長文要約、Markdown生成を、すべてローカルPCで行う想定だった。その後、Pixelなど別の端末・アプリで文字起こしを済ませ、PCにはテキストだけを渡す案が出たことで、PCに求める性能と予算の考え方が変わった。

> **すでに文字起こし済みのテキストを要約してMarkdown化する用途であれば、音声認識用の高性能GPUは必須ではない。** この用途では、音声処理AIではなく文章整理AIとしてローカルLLMを使うため、7B〜14B級を無理なく動かせる環境が品質と費用の比較軸になる。

この結論は、動画から音声認識までPC側で行う場合にはそのまま当てはまらない。以下では、前提が変わる前後の処理経路を分けて、なぜ必要性能が変わるのかを追う。

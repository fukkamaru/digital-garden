...
調査上の補助ノートは、[ストレージ関連エラー イベントID7](storage-event-id-7.md)、[ユーザープロファイルを読み込めなくなる原因候補](user-profile-load-failure-causes.md)、[ユーザープロファイルを復旧するか、諦めるかの判断基準](user-profile-recovery-decision.md)へ分けた。

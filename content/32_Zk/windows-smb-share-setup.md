...
このノートは、Windows PC上の特定フォルダーをSMB共有として公開する設定手順である。利用者の接続は[SMB共有フォルダへ接続する方法](SMB共有フォルダへ接続する方法.md)、既存共有の確認・運用は[WindowsのSMB共有を管理する方法](windows-smb-share-administration.md)を参照する。

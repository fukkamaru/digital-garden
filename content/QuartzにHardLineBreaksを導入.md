...
代わりに[[hard line break]]におけるいずれかの手法を使うことで、
段落を変えずに改行することが出来る。
...
直前に置いたリンク先の記事を参考に説明する。

■ソースモード
![](screen-shot-2026-05-03%20195245.png)

■Quartz（ウェブ）上での表示
![](screen-shot-2026-05-03%20201252.png)
...
Markdownテキスト内の単一改行をHTML出力で強制改行に自動的に変換するために、「HardLineBreaks」プラグインが標準実装されているらしい。しかし、Markdwonの標準的なセマンティクス（意図）に準拠していないため、デフォルトではOFFになっている。

わざわざ\<br\>を入力しなくても、単一改行でHTML上の体裁を整えてくれるのは大変便利な機能。わっちはマークダウン原理主義者ではないし、日本語ユーザーなのもあって文章中に改行する方が読みやすくプラグインを使うことにした。
